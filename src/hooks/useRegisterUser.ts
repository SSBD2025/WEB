        ...
        onError: (error: unknown) => {
            if (axios.isAxiosError(error)) {
                ...
                if (status === 409) {
                    ...
                    } else if (data === "Constraint error: this login is already in use") {
                    ...
                    if (Array.isArray(violations)) {
                        const passwordViolation = violations.find(v => v.fieldName === "account.password");
                        if (passwordViolation) {
                            toast.error(t("register.error.passwordRegex"));
                            return;
                        }
                        const firstNameViolation = violations.find(v => v.fieldName === "account.firstName");
                        if (firstNameViolation) {
                            toast.error(t("register.error.firstNameRegex"));
                            return;
                        }
                        const lastNameViolation = violations.find(v => v.fieldName === "account.lastName");
                        if (lastNameViolation) {
                            toast.error(t("register.error.lastNameRegex"));
                            return;
                        }
                        const loginViolation = violations.find(v => v.fieldName === "account.login");
                        if (loginViolation) {
