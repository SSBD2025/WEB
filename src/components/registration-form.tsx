...
export function RegistrationForm({ userType }: RegistrationFormProps) {
  ...
  const formSchema = z
    .object({
      firstName: z
        .string()
        .min(2, t("register.firstname.too_short"))
        .max(50, t("register.firstname.too_long")),
      lastName: z
        .string()
        .min(2, t("register.lastname.too_short"))
        .max(50, t("register.lastname.too_long")),
      login: z
        .string()
        .min(4, t("register.login.too_short"))
        .max(50, t("register.login.too_long")),
      email: z
        .string()
        .email(t("register.email.invalid"))
        .max(50, t("register.email.too_long")),
      password: z
        .string()
        .min(8, t("register.password.too_short"))
        .max(60, t("register.password.too_long")),
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                render={({ field }) => (
                  <FormItem>
                    <RequiredFormLabel htmlFor="lastName" >
                      {t("register.lastname.field")}
                    </RequiredFormLabel>
                    <FormControl>
                      <Input
                        placeholder={t("register.lastname.field")}
                        {...field}
                      />
                    </FormControl>
                    <FormMessage />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <RequiredFormLabel htmlFor="login" >
                    {t("register.login.field")}
                  </RequiredFormLabel>
                  <FormControl>
              ...
              render={({ field }) => (
                <FormItem>
                  <RequiredFormLabel htmlFor="confirmPassword" >
                    {t("register.confirm_password.field")}
                  </RequiredFormLabel>
                  <FormControl>
                    <Input
                      placeholder={t("register.confirm_password.field")}
                      type="password"
                      {...field}
                    />
                  </FormControl>
