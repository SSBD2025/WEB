...
export default function CreateDietProfile() {
  ...
  } = useGetClientPermanentSurvey(clientId!);
  ...
  } = useGetAllDieticianPeriodicSurvey(
    {
      page: page - 1,
      size: pageSize,
      since,
  ...
  const [selectedExisting, setSelectedExisting] = useState<FoodPyramid | null>(
    null
  );
  const {
    data: clientBloodTestReports,
    error: clientBloodTestReportsError,
    status: clientBloodTestReportsStatus,
  } = useClientBloodTestByDieticianReports(clientId);
  const {
    data: foodPyramidsDataList,
    error: foodPyramidsError,
    status: foodPyramidsStatus,
  } = useGetAllFoodPyramids();
  const saveMutation = useCreateNewDietPlan(clientId!);

  const { data: clientDetails } = useClientValidation(clientId!);
  ...
  useEffect(() => {
    ...
    const storedLocale = localStorage.getItem("i18nextLng") || "pl-PL";
  ...
  const [selectedSurveyFromModal, setSelectedSurveyFromModal] =
  ...
  const copyFromExisting = () => {
    ...
    for (const key in selectedExisting) {
      if (
        typeof selectedExisting[key as keyof typeof selectedExisting] ===
        "number"
      ) {
        newWorkspace[key] = String(
          selectedExisting[key as keyof typeof selectedExisting]
        );
      }
    }
  ...
  if (
    ...
    foodPyramidsStatus === "pending" ||
    periodicSurveyStatus === "pending"
  ) {
    return (
  ...
  if (clientDataError || clientBloodTestReportsError || periodicSurveyError) {
    return (
      <div className="flex items-center justify-center flex-col flex-grow gap-3">
        {clientBloodTestReportsError && (
            <p>{t("create_diet_profile.no_blood_test_reports")}</p>
        )}
        {periodicSurveyError && (
            <p>{t("create_diet_profile.no_periodic_survey")}</p>
        )}
        {clientDataError && <p>{t("create_diet_profile.no_permanent_survey")}</p>}
      </div>
    );
  }
              ...
              <Card className="h-full">
                <CardHeader>
                  <CardTitle className="text-lg">
                    {t("create_diet_profile.permanent_survey.table_name")}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-5 gap-3 mb-4 text-sm">
                    <div>
                      <div className="text-xs font-medium text-muted-foreground">
                        {t("create_diet_profile.permanent_survey.name")}
                      </div>
                      <div className="font-medium">
                        {clientDetails?.account.firstName}{" "}
                        {clientDetails?.account.lastName}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs font-medium text-muted-foreground">
                        {t("create_diet_profile.permanent_survey.height")}
                      </div>
                    ...
                    <div>
                      <div className="text-xs font-medium text-muted-foreground">
                        {t(
                          "create_diet_profile.permanent_survey.activity_level"
                        )}
                      </div>
                      <div className="font-medium">
                        {t(
                          `permanent_survey_form.activity_level_name.${clientData?.activityLevel}`
                        )}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs font-medium text-muted-foreground">
                        {t("create_diet_profile.permanent_survey.age.age")}
                      </div>
                      <div className="font-medium">
                        {" "}
                        {clientData?.dateOfBirth
                          ? Math.floor(
                              (Date.now() -
                                new Date(clientData.dateOfBirth).getTime()) /
                                (1000 * 60 * 60 * 24 * 365.25)
                            )
                          : t("create_diet_profile.age.no_date_of_birth")}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs font-medium text-muted-foreground">
                        {t(
                          "create_diet_profile.permanent_survey.smokes.smokes"
                        )}
                      </div>
                      <div className="font-medium">
                        {clientData?.smokes
                          ? t(
                              "create_diet_profile.permanent_survey.smokes.true"
                            )
                          : t(
                              "create_diet_profile.permanent_survey.smokes.false"
                            )}
                      </div>
                    </div>
                  </div>
                  <div className="grid grid-cols-5 gap-3 mb-4 text-sm">
                    <div>
                      <div className="text-xs font-medium text-muted-foreground">
                        {t(
                          "create_diet_profile.permanent_survey.gender.gender"
                        )}
                      </div>
                      <div className="font-medium">
                        {clientData?.gender
                          ? t(
                              "create_diet_profile.permanent_survey.gender.male"
                            )
                          : t(
                              "create_diet_profile.permanent_survey.gender.female"
                            )}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs font-medium text-muted-foreground">
                        {t(
                          "create_diet_profile.permanent_survey.drinks_alcohol.drinks"
                        )}
                      </div>
                      <div className="font-medium">
                        {clientData?.drinksAlcohol
                          ? t(
                              "create_diet_profile.permanent_survey.drinks_alcohol.true"
                            )
                          : t(
                              "create_diet_profile.permanent_survey.drinks_alcohol.false"
                            )}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs font-medium text-muted-foreground">
                        {t(
                          "create_diet_profile.permanent_survey.meals_per_day"
                        )}
                      </div>
                      <div className="font-medium">
                        {clientData?.mealsPerDay}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs font-medium text-muted-foreground">
                        {t(
                          "create_diet_profile.permanent_survey.nutrition_goal"
                        )}
                      </div>
                      <div className="font-medium">
                        {t(
                          `permanent_survey_form.${clientData?.nutritionGoal.toLowerCase()}`
                        )}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs font-medium text-muted-foreground">
                        {t(
                          "create_diet_profile.permanent_survey.eating_habits"
                        )}
                      </div>
                      <div className="font-medium">
                        {clientData?.eatingHabits}
                      </div>
                    </div>
                  </div>
                  <div className="grid grid-cols-5 gap-3 mb-4 text-sm">
                    <div>
                      <div className="text-xs font-medium text-muted-foreground">
                        {t("create_diet_profile.permanent_survey.medications")}
                      </div>
                      <div className="font-medium text-xs">
                        {clientData?.medications.map((medication) => (
                          <p>{medication}</p>
                        ))}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs font-medium text-muted-foreground">
                        {t("create_diet_profile.permanent_survey.illnesses")}
                      </div>
                      <div className="font-medium text-xs">
                        {clientData?.illnesses.map((illness) => (
                          <p>{illness}</p>
                        ))}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs font-medium text-muted-foreground">
                        {t("create_diet_profile.permanent_survey.allergies")}
                      </div>
                      <div className="font-medium text-xs">
                    ...
                    <div>
                      <div className="text-xs font-medium text-muted-foreground">
                        {t(
                          "create_diet_profile.permanent_survey.diet_preferences"
                        )}
                      </div>
                      <div className="font-medium text-xs">
                        {clientData?.dietPreferences.map((preference) => (
                          <p>{preference}</p>
                        ))}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs font-medium text-muted-foreground">
                        {t("create_diet_profile.permanent_survey.meal_times")}
                      </div>
                      <div className="font-medium text-xs">
                        {clientData?.mealTimes.map((mealTime) => (
                          <p>{formatTime(new Date(mealTime))}</p>
                        ))}
                      </div>
                    </div>
                  </div>
                  ...
                  <CardTitle className="text-lg flex items-center justify-between">
                    ...
                    {totalSurveys > 1 && (
                      <>
                        ...
                        >
                          <Pencil />
                          {t("create_diet_profile.periodic_survey.list")}
                        </Button>
                        <span className="text-sm font-normal text-muted-foreground">
                          {currentIndexSurvey + 1}{" "}
                          {t("create_diet_profile.periodic_survey.from")}{" "}
                          {totalSurveys}
                        </span>
                      </>
                    )}
                ...
                <CardContent className="h-full flex flex-col flex-1">
                  <div className="grid grid-cols-3 gap-3 mb-4 text-sm">
                    <div>
                      <div className="text-xs font-medium text-muted-foreground">
                        {t(
                          "create_diet_profile.periodic_survey.blood_sugar_level"
                        )}
                      </div>
                      <div className="font-medium">
                    ...
                    <div>
                      <div className="text-xs font-medium text-muted-foreground">
                        {t(
                          "create_diet_profile.periodic_survey.blood_pressure"
                        )}
                      </div>
                      <div className="font-medium">
                    ...
                    <div className="text-center">
                      {currentSurvey?.measurementDate && (
                        <div className="font-medium text-sm">
                          {t(
                            "create_diet_profile.periodic_survey.measurement_date"
                          )}{" "}
                          {formatDateTime(currentSurvey.measurementDate)}
                        </div>
                      )}
                    </div>
            ...
            render={() => (
              <Card className="h-full">
                <CardHeader className="pb-3">
                  <CardTitle className="text-lg">
                    {t("create_diet_profile.existing_profiles_list.table_name")}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                    ...
                    {foodPyramidsDataList?.map((pyramid, index) => (
                      <div
                        ...
                        className={`flex items-center justify-between p-2 rounded-md cursor-pointer transition-colors
                                ${
                                  selectedExisting?.id === pyramid.id
                                    ? "bg-muted"
                                    : "hover:bg-accent"
                                }`}
                      >
                        ...
                        <span className="flex items-center text-sm text-muted-foreground">
                          {pyramid.averageRating?.toFixed(1) ?? "-"}
          ...
          <Card className="h-full">
            <CardHeader className="pb-3">
              <CardTitle className="text-lg flex flex-row justify-between items-center w-full">
                <span>
                  {t("create_diet_profile.algorithm_pane.table_name")}
                </span>
            ...
            <CardContent className="space-y-2 h-155 overflow-y-auto">
              <div className="space-y-2 h-155 overflow-y-auto">
                <Table className="table-fixed">
                  <TableHeader>
                    <TableRow>
                      <TableHead className="text-left text-lg w-24">
                        {t("create_diet_profile.algorithm_pane.parameter_name")}
                      </TableHead>
                      <TableHead className="text-center text-lg w-24">
                        {t("create_diet_profile.algorithm_pane.existing")}
                      </TableHead>
                      ...
                      <TableHead className="text-center text-lg w-24">
                        {t("create_diet_profile.algorithm_pane.algorithm")}
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {nutrientRows.map((row) => (
                      <TableRow key={row.name}>
                          ...
                          <div key={row.name} className="relative">
                            ...
                            {row.workspace && (
                              <Button
                                variant="ghost"
                                onClick={() =>
                                  handleWorkspaceChange(row.name, "")
                                }
                        ...
                        <TableCell className="text-medium text-center w-24 truncate">
                          {row.algorithm}
                        </TableCell>
                      </TableRow>
                    ))}
            ...
            render={() => (
              <Card className="h-full flex flex-col">
                <CardHeader>
                  <CardTitle className="text-lg flex items-center justify-between">
                    ...
                    {totalBlood > 1 && (
                      <span className="text-sm font-normal text-muted-foreground">
                        {currentIndexBlood + 1}{" "}
                        {t("create_diet_profile.blood_test_reports.from")}{" "}
                        {totalBlood}
                      </span>
                    )}
                  </CardTitle>
                ...
                <CardContent className="h-full flex-col flex-1">
                  {clientBloodTestReportsStatus === "success" &&
                    currentBlood?.results && (
                      <div className="flex flex-col h-full">
                        <div className="grid grid-cols-4 gap-3 mb-4 text-sm border-b ml-1 mr-1">
                          <div className="text-medium font-medium">
                            {t(
                              "create_diet_profile.blood_test_reports.parameter"
                            )}
                          </div>
                          <div className="text-medium font-medium">
                            {t("create_diet_profile.blood_test_reports.result")}
                          </div>
                          <div className="text-medium font-medium">
                            {t("create_diet_profile.blood_test_reports.norm")}
                          </div>
                          <div className="text-medium font-medium">
                            {t("create_diet_profile.blood_test_reports.status")}
                          </div>
                        </div>
                        <div className="h-140 overflow-y-auto ml-1 mr-1">
                          {currentBlood.results.map((currentBloodParam) => {
                            ...
                            return (
                              <div
                                key={currentBloodParam.lockToken}
                              ...
                              >
                                ...
                                <div className="text-sm font-medium">
                                  ...
                                  {currentBloodParam.bloodParameter.standardMax}{" "}
                                  {formatUnit(
                                    currentBloodParam.bloodParameter.unit
                                  )}
                                </div>
                                <Badge
                        ...
                        <div className="flex flex-row items-center justify-between mt-auto">
                          ...
                          <div className="text-center">
                            {currentBlood?.timestamp && (
                              <div className="font-medium text-sm">
                                {t(
                                  "create_diet_profile.blood_test_reports.measurement_date"
                                )}{" "}
                                {formatDateTime(
                                  new Date(currentBlood.timestamp)
                                )}
                              </div>
                            )}
                          </div>
